...
gr.bar.freq <- function(var_y, var_x = 'ola', data = elsoc_panel_m1, ponderador = 'ponderador02',
  ...
  ggp <- ggplot(datos.grafico, 
    ...
    theme(legend.position = 'top', 
          ...
          legend.text = element_text(size = rel(ajustar_texto(levels(getElement(datos.grafico, var_y)), return_tamano = TRUE))),
          axis.text.x = element_text(size = rel(ajustar_texto(levels(getElement(datos.grafico, var_x)), return_tamano = TRUE)))
          )
...
gr.line.freq.rec.list <- function(var_y, var_x = 'ola', REC = NULL,  data = elsoc_panel_m1, 
  ...
  # Etiqueta de valores de eje Y si se definen
  if (!is.null(label_y)) {
    largo_max_label_y <- round(120/length(label_y),0)
    label_y_cut <- lapply(label_y, cortar_texto, largo = largo_max_label_y)
     ggp <- ggp + scale_colour_discrete(labels = label_y_cut)
