...
elsoc_long$empleo <- factor(elsoc_long$empleo, 
                            labels = c("Trabajo remunerado", "Desempleado", "Trabajo no remunerado", "Jubilado o pensionado", "Otros inactivos"))
...
elsoc_long$mov <- factor(elsoc_long$c20,
                             labels = c('Estudiantil',
                                        ...
                                        'Otro',
                                        'Ninguno'))
elsoc_long$mov <- factor(elsoc_long$mov,
...
elsoc_long <- elsoc_long %>% 
    mutate(confianza = factor(ifelse(ola == 2016 | ola == 2018, c06_06, r16)),
           norma = case_when(as.numeric(r12_01) + as.numeric(r12_02) <= 3 ~ 1,
...
elsoc_long$clase.sub2 <- car::recode(as.numeric(elsoc_long$d01_01), recodes = "c(0,1,2)=1; c(3,4)=2; c(5,6)=3; c(7,8)=4; c(9,10)=5; else=NA", as.factor = TRUE)

elsoc_long$clase.sub2 <- factor(elsoc_long$clase.sub2,
                                levels = c(1,2,3,4,5),
                                labels = c("Mas Bajo", "Bajo", "Medio", "Alto", "Mas Alto"))
elsoc_long$clase.sub2 <- sjlabelled::set_label(elsoc_long$clase.sub2, "Lugar subjetivo en la sociedad chilena")
elsoc_long$clase.sub2 <- sjlabelled::set_labels(elsoc_long$clase.sub2, labels = c("Mas Bajo", "Bajo", "Medio", "Alto", "Mas Alto"))


#--------------------N.- Clase subjetiva en tres categorias-------------------------
elsoc_long$clase.sub <- car::recode(as.numeric(elsoc_long$d01_01), recodes = "c(0,1,2,3,4)=1; c(5)=2; c(6,7,8,9,10)=3;else=NA", as.factor = TRUE)
...
#-------------------CLASE SUBJETIVA-------------------------------
elsoc_long$clase.sub2.hijos <- car::recode(as.numeric(elsoc_long$d01_03), recodes = "c(0,1,2)=1; c(3,4)=2; c(5,6)=3; c(7,8)=4; c(9,10)=5; else=NA", as.factor = TRUE)

elsoc_long$clase.sub2.hijos <- factor(elsoc_long$clase.sub2.hijos,
                                levels = c(1,2,3,4,5),
                                labels = c("Mas Bajo", "Bajo", "Medio", "Alto", "Mas Alto"))
elsoc_long$clase.sub2.hijos <- sjlabelled::set_label(elsoc_long$clase.sub2.hijos, "Ubicacion subjetiva de sus hijos en la sociedad chilena")
elsoc_long$clase.sub2.hijos <- sjlabelled::set_labels(elsoc_long$clase.sub2.hijos, labels = c("Mas Bajo", "Bajo", "Medio", "Alto", "Mas Alto"))
...
rm(dir_carlos, dir_edgardo, var_labels, pasar_a_factor, elsoc_long_2016_2019)
