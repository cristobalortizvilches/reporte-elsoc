...
gr.bar.freq <- function(var_y, var_x = 'ola', data = elsoc_panel, ponderador = 'ponderador02',
  ...
  if (!is.null(titulo)) {
    ggp <- ggp + 
      ggtitle(title = cortar_texto(titulo, largo = 70))
  }
  print(ggp)
}
...
save(gr.alluvial.freq.facet, gr.bar.freq, gr.bar.freq.rec, gr.bar.media, gr.line.freq, gr.line.freq.rec, gr.line.media, gr.alluvial.freq, tb.freq, tb.freq.rec, cortar_texto,
