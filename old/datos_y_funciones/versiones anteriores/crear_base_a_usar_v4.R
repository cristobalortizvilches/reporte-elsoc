...
#Recodificar posicionamiento ideológico
elsoc_long$pospolitico <- car::recode(elsoc_long$c15, recodes = "c(0,1,2,3, 4)=1; c(5)=2; c(6,7,8,9,10)=3; c(11,12)=4; else=NA", as.factor = TRUE)
#Pasar a factor
elsoc_long$pospolitico <- factor(elsoc_long$pospolitico,
                            levels = c(1, 2, 3, 4),
                            labels = c("Izquierda", "Centro", "Derecha", "Ninguno"))
#Atributos
elsoc_long$pospolitico <- sjlabelled::set_label(elsoc_long$pospolitico, "Identificación Política")
elsoc_long$pospolitico <- sjlabelled::set_labels(elsoc_long$pospolitico, labels = c("Izquierda", "Centro", "Derecha", "Ninguno"))
...
elsoc_long$acuerdo_cambio_constitucion <- factor(with(elsoc_long, case_when(
    c28 == 'Totalmente en desacuerdo' | c26 == 'En desacuerdo' ~ 1,
    c28 == 'Ni de acuerdo ni en desacuerdo' ~ 2,
    c28 == 'De acuerdo' | c26 == 'Totalmente de acuerdo' ~ 3)),
...
elsoc_panel_m1 <- dplyr::filter(elsoc_long, muestra == 'Muestra Original')
elsoc_panel_m2 <- dplyr::filter(elsoc_long, muestra == 'Muestra Refresco' )

# Base sin pasar a factor:
elsoc_panel_sf <- sjlabelled::set_na(elsoc_long_2016_2019, na = c(-888, -999))
elsoc_panel_m1_sf <- dplyr::filter(elsoc_panel_sf, muestra == 1)
elsoc_panel_m2_sf <- dplyr::filter(elsoc_panel_sf, muestra == 2)
