...
x <- c('asd', 'oqiweuoqwiej', 'oijoasdj oei1092 ')
3/(1 + log(max(nchar('abcdaasdaasdasdsadasdadasdasdasasdasdasdasdsdasd'))))

# Ajustar largo y tamaño de texto de lista de leyendas/etiquetas:
ajustar_texto <- function(texto){
  
  max <- max(nchar(texto))
  tamanno_relativo <- 1 + log(max)
...
gr.bar.freq <- function(var_y, var_x = 'ola', data = elsoc_panel_m1, ponderador = 'ponderador02',
  ...
  largo_max_label_x <- round(140/length(unique(getElement(datos.grafico, var_x))),0) - 10
  largo_max_label_y <- round(140/length(unique(getElement(datos.grafico, var_y))),0) 
...
gr.bar.freq.list <- function(var_x, REC = NULL,  data = elsoc_panel_m1, 
  ...
  largo_max_label_z <- round(120/length(unique(datos.grafico$categoria)),0) 
...
gr.bar.media.list <-  function(var_x, var_z = 'ola', data = elsoc_panel_m1, 
  ...
  largo_max_label_z <- round(120/length(unique(getElement(data, var_z))), 0) 
  # Si eje Z tiene un único nivel, se usan colores según eje X, para que no se vea tan plano
...
save(gr.bar.mediana, gr.alluvial.freq.facet, gr.bar.media.list, gr.bar.freq, gr.bar.freq.rec, gr.bar.media, gr.line.freq, gr.line.freq.rec, gr.line.media, gr.alluvial.freq, tb.freq, tb.freq.rec, gr.bar.freq.list, gr.bar.freq.rec.list, gr.line.freq.rec.list, cortar_texto,
